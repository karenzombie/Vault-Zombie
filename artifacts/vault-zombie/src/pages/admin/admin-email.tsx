  ...
  return <div className="space-y-4">
    ...
    <div className="bg-white border border-border rounded-xl overflow-x-auto"><table className="w-full text-sm">
      ...
      <tbody>{data?.deliveries.map((delivery) => <tr key={delivery.id} className="border-t">
        ...
        <td className="p-3">{delivery.attempts}{delivery.lastError ? ` — ${delivery.lastError}` : ""}</td><td className="p-3">{delivery.status === "failed" && <Button size="sm" disabled={!reason.trim() || retry.isPending} onClick={async () => { await runSensitive(() => retry.mutateAsync({ emailDeliveryId: delivery.id, data: { reason } })); queryClient.invalidateQueries({ queryKey: getListAdminEmailDeliveriesQueryKey() }); }}>Retry</Button>}</td>
      </tr>)}</tbody>
    </table></div>
  </div>;
}